#!/usr/bin/env python3
"""Compare a saved Qwen post-training result with the head-only Qwen baseline."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--baseline-label", default="frozen-Qwen+photo-z-head"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for role, path in (
        ("Baseline", args.baseline_result_path),
        ("Post-training", args.result_path),
    ):
        if not path.is_file():
            raise FileNotFoundError(f"{role} result not found: {path}")
    baseline = torch.load(
        args.baseline_result_path, map_location="cpu", weights_only=False
    )
