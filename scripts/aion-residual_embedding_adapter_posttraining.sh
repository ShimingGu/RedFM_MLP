...
GPU_DEVICE="${AION_ADAPTER_GPU_DEVICE:-3}"
OUTPUT_ROOT="${AION_ADAPTER_OUTPUT_DIR:-/arc/home/gsm/aion_output/figures/aion-residual_embedding_adapter-e10}"
...
for INPUT_MODE in "${INPUT_MODES[@]}"; do
    ...
    mkdir -p -- "$OUTPUT_DIR"
    echo "AION residual-adapter GPU slice: $GPU_DEVICE"
    ...
    "${PYTHON_CMD[@]}" "$REPO_ROOT/notebooks/plot_aion_posttraining.py" \
        ...
        --prefix "aion_${MODE_TAG}_embedding_adapter_comparison" \
        --baseline-label "mean-pooled AION head control ($INPUT_MODE)" \
        --label "post-encoder vector adapter control ($INPUT_MODE)" \
